...
def calculate_regularization(model, l1_lambda=1e-5, l2_lambda=1e-5, device="cuda"):
    l1_reg = 0
    l2_reg = 0

    # L1 正则化：对所有模型参数计算绝对值和
...
def train_one_epoch(
    ...
    for features, labels in iterator:
        ...
        # 计算L1和L2正则化
        l1_l2_reg = calculate_regularization(
            model, l1_lambda=1e-5, l2_lambda=1e-5, device=device
        )

        # 总损失 = 原始损失 + 正则化损失
        total_loss = loss + l1_l2_reg
