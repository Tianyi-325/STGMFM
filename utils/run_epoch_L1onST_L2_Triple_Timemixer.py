...
# def _compute_l2(
#     model: torch.nn.Module, device_main: torch.device, l2_lambda: float = 1e-5
# ) -> torch.Tensor:
#     l2 = torch.tensor(0.0, device=device_main)
#     for p in model.parameters():
#         if p.requires_grad:
#             l2 = l2 + torch.sum(p.pow(2).to(device_main))  # 关键：to(device_main)
#     return l2_lambda * l2


# def _compute_l1_graph(
#     model: torch.nn.Module, device_main: torch.device, l1_lambda: float = 1e-5
# ) -> torch.Tensor:
#     """L1 sparsity penalty over *all* learnable graph parameters."""
#     l1 = torch.tensor(0.0, device=device_main)
...
def train_one_epoch(
    ...
    for step, (features, labels) in enumerate(iterator):
        ...
        # λ_reg = 0.01
        # total_loss = ce_loss + l2_reg + l1_reg + λ_reg * model.fuser.gate.abs().sum()
    ...
    # Return the CCG matrices for plotting
    ccg_w_A = ccg_w_A.detach().cpu().numpy()
    ccg_w_B = ccg_w_B.detach().cpu().numpy()
