...
if __name__ == "__main__":
    ...
    # 1️⃣  构造 configs —— 只要把代码里会用到的字段补齐即可
    cfg = SimpleNamespace(
        # ---- 数据与任务 ----
        task_name="classification",  # 任务类型：classification / long_term_forecast ...
        num_class=3,  # 分类类别数
        enc_in=22,  # 输入通道数（EEG 电极数）
        seq_len=125,  # 单条序列长度
        label_len=0,  # 仅预测任务会用，留 0 即可
        pred_len=0,  # 仅预测任务会用，留 0 即可
        # ---- PDM / 多尺度混合相关 ----
        down_sampling_window=2,
        down_sampling_layers=3,
        e_layers=2,  # PastDecomposableMixing 堆叠层数
        moving_avg=5,
        decomp_method="moving_avg",  # 或 'dft_decomp'
        top_k=5,  # 仅 dft_decomp 用
        # ---- 通道设置 ----
        channel_independence=0,  # 是否把每个通道当独立 batch
        c_out=23,  # 非分类任务用到，先填 enc_in
        # ---- 网络宽度/深度 ----
        d_model=64,
        d_ff=128,
        embed="timeF",  # DataEmbedding_wo_pos 需要的占位符
        freq="h",  # 同上
        dropout=0.3,
        # ---- 其它 ----
        use_future_temporal_feature=False,
        use_norm=1,
        down_sampling_method="max",  # 'max' / 'avg' / 'conv'
    )

    # 2️⃣  构建模型
    model = Model(cfg)

    # 3️⃣  伪造一段 EEG 输入：shape = (batch, 时间点, 通道)
    B = 32
    x_enc = torch.randn(B, cfg.seq_len, cfg.enc_in)  # (B, T, C)
    x_mark_enc = torch.ones(B, cfg.seq_len)  # 时间掩码；分类可全 1
    # 预测 / 插补任务才用到，下方占位即可
    x_dec = x_mark_dec = None

    # 4️⃣  前向传播
    logits = model(x_enc, x_mark_enc, x_dec, x_mark_dec)  # -> (B, num_class)

    print("logits shape:", logits.shape)  # 期望 (4, 4)
