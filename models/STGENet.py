...
class SpatialGraph(nn.Module):
    def __init__(self, n_nodes=22, adj=None, k=2, spatial_GCN=True, device=0):
        super(SpatialGraph, self).__init__()
        self.xs, self.ys = torch.tril_indices(
            n_nodes, n_nodes, offset=-1
        )  # offset=-1:不包含对角线的下三角部分
        node_value = adj[self.xs, self.ys]
        self.edge_weight = nn.Parameter(
            node_value.clone().detach(), requires_grad=spatial_GCN
        )
    ...
    def forward(self, x):
        if not (self.spatial_GCN):
            edge_weight = torch.eye(
                self.n_nodes, self.n_nodes
            )  # 对角线为1的矩阵，确保自连接
        else:
            edge_weight = torch.zeros([self.n_nodes, self.n_nodes], device=self.device)
            edge_weight[self.xs.to(self.device), self.ys.to(self.device)] = (
                self.edge_weight.to(self.device)
            )
            edge_weight = (
                edge_weight
                + edge_weight.T
                + torch.eye(self.n_nodes, dtype=edge_weight.dtype, device=self.device)
            )
            edge_weight = normalize(edge_weight)
            edge_weight = edge_weight.cuda(self.device)
            edge_weight = edge_weight + torch.eye(self.n_nodes, self.n_nodes).cuda(
                self.device
            )
...
class TimeGraph(nn.Module):
    def __init__(self, window, k, channels, time_GCN=True):
        ...
        self.adj = nn.Parameter(
            0.5 * torch.ones(window, window) + 1.5 * torch.eye(window, window),
...
class GraphAttention(nn.Module):
    def __init__(self, nodes, time_length):
        ...
        self.a = nn.Parameter(
            torch.empty(size=(2 * time_length, 1)), requires_grad=True
        )
    ...
    def forward(self, x):
        ...
        for t in x:
            ...
            attention = torch.matmul(
                torch.diag(self.node_weight).repeat(temp.shape[0], 1, 1),
                torch.matmul(attention, torch.diag(self.node_weight)),
            ) + torch.matmul(
                torch.diag(self.node_weight).repeat(temp.shape[0], 1, 1),
                torch.matmul(10 * I.cuda(1), torch.diag(self.node_weight)),
            )
    ...
    def __init__(
        ...
        for _ in range(self.layers):
            self.timeconv.append(
                Conv(
                    nn.Conv1d(
                        out_chans, out_chans, kernel_size, 1, padding="same", bias=False
                    ),
                    bn=nn.BatchNorm1d(out_chans),
                    activation=None,
                )
        ...
        self.downSampling = nn.AvgPool1d(
            int(sampling_rate // 2), int(sampling_rate // 2)
        )
        self.dp = nn.Dropout(dropout)
        self.fc = nn.Sequential(
            nn.Linear(
                out_chans * (input_time_length * slide_window // (sampling_rate // 2)),
                n_classes,
            )
        )
        ...
        self.ge = nn.Sequential(
            SpatialGraph(
                ...
                adj=Adj,
                k=k_spatial,
                spatial_GCN=spatial_GCN,
                device=device,
            ),
        )

        self.ge_space = nn.Sequential(
            TimeGraph(slide_window, k_time, out_chans, time_GCN)
        )
        ...
        self.GAT = GraphAttention(
            nodes=in_chans, time_length=input_time_length * slide_window
        )
        ...
        self.separable_conv = nn.Sequential(
            ...
            Conv2dWithConstraint(
                (64), 16, (1, 1), max_norm=None, stride=1, bias=False, padding=(0, 0)
            ),
            nn.BatchNorm2d(16, momentum=0.01, affine=True, eps=1e-3),
        )
        self.cls = nn.Sequential(
            Conv2dWithConstraint(
                16, self.n_classes, (1125, 1), max_norm=0.25, bias=True
            ),
        )
        ############################################################################################################################
