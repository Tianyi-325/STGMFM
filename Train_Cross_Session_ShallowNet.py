# ===== Train_Cross_Session_ShallowNet.py =====
import os
import re
import argparse
import random
...
# Cross-Session 数据接口（同人 session1->train, session2->test 修正）
from utils.Dataload_Cross_Session_Filter import create_dataset_for_cross_validation

# ShallowNet 模型（你上传的实现）
...
def run_one(args, sid: int):
    """训练/评估一个受试者（严格单设备；日志命名用受试者姓名）。"""
    # 建议让每位受试者的 seed 稍作偏移，避免曲线“视觉上完全一致”
    ...
    # 动态获取 T（样本长度），对齐 ShallowNet 的 input_window_samples
    T = train_loader.dataset.tensors[0].shape[-1]

    # ===== 构建 ShallowNet（仅 logits 输出）=====
    model = ShallowNet(
    ...
    # 优化器 / 损失 / 调度器（我们不用 Lightning 的 configure_optimizers）
    ...
    if swanlab is not None:
        try:
            swanlab.init(
                project="Main-ShallowNet-Cross-Session_Filter",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc > best_acc:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(sub_logdir, f"best_shallownet_cs_{subj_name}.pth"),
            )
...
def main():
    parser = argparse.ArgumentParser(
        "Cross-Session Training for ShallowNet (logits-only, single device)"
    )
    parser.add_argument(
    ...
    parser.add_argument("-logdir", type=str, default="./runs_shallownet_cs")
    # —— ShallowNet 结构超参（与你文件中的默认一致）——
